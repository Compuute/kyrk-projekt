...
| **Kategorier av personuppgifter** | Ingen spårning, inga analysverktyg, ingen insamling till databas. Två funktionella kakor lagrar besökarens egna val i webbläsaren: språk (`selected_language`) och vald församling (`selected_church`); de läses av Cloudflare-edgen för att rendera rätt innehåll. OBS: valet av församling kan indirekt antyda religiös tillhörighet — klassning enligt Art. 9 ska bedömas av personuppgiftsansvarig. **Anonym aggregerad mätning är byggd men VILANDE** (`METRICS_ENABLED = false` i `app.ts`; ej deployad, inget KV-namespace skapat) — den samlar **ingenting** i nuläget. Om den aktiveras: rena dygnsräknare utan IP/identifierare (`functions/m.ts` → `kyrka_metrics` KV, YELLOW). Beslut att avvakta: `docs/26 §Status`. |
| **Rättslig grund** | Funktionella kakor för en tjänst användaren själv begärt (undantagna samtyckeskravet enligt ePrivacy art. 5.3); ingen övrig personuppgiftsbehandling i nuläget. ⚠️ **Om** den vilande mätningen aktiveras skriver den till `localStorage`, vilket enligt ePrivacy art. 5.3 normalt **inte** är "strikt nödvändigt" och kan kräva samtycke/information — **personuppgiftsansvarig ska ta ställning före aktivering**. *Denna rad uppdaterad — ska granskas av personuppgiftsansvarig.* |
| **Tredjelandsöverföring** | Cloudflare CDN (EU edge). Ingen persondata cachas. |
| **System** | frontend/member-portal (GREEN), frontend/wifi-intake-portal (GREEN); metrics-beacon `functions/m.ts` (YELLOW, vilande) |
