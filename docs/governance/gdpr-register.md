...
| **Kategorier av personuppgifter** | Inga tredjepartsverktyg, ingen identifierare, ingen koppling till en individ. Två funktionella kakor lagrar besökarens egna val i webbläsaren: språk (`selected_language`) och vald församling (`selected_church`); de läses av Cloudflare-edgen för att rendera rätt innehåll. OBS: valet av församling kan indirekt antyda religiös tillhörighet — klassning enligt Art. 9 ska bedömas av personuppgiftsansvarig. **Anonym aggregerad mätning** (`functions/m.ts` → `kyrka_metrics` KV, zon YELLOW): rena dygnsräknare för PWA-installation, push-opt-in och retention. Endast ett ord ur en fast allowlist skickas per händelse (`{ "e": "app_open" }`); **ingen IP, ingen identifierare, ingen sidväg** lagras — edgen kan inte koppla två anrop till samma enhet. Räknare självdör efter 90 dagar. Beslutsunderlag: `docs/26`. |
| **Rättslig grund** | Funktionella kakor för en tjänst användaren själv begärt (undantagna samtyckeskravet enligt ePrivacy art. 5.3). Den aggregerade mätningen behandlar **ingen personuppgift** (GDPR ej tillämplig på anonyma aggregat) — men den skriver till `localStorage` för avdubblering, vilket enligt ePrivacy art. 5.3 normalt **inte** är "strikt nödvändigt" och kan kräva samtycke/information. Mildras via `Do-Not-Track`-respekt + lokalt opt-out (`metricsOptOut`). ⚠️ **Personuppgiftsansvarig ska ta ställning** till om en lättviktig samtyckes-/informationsrad behövs. *Denna rad uppdaterad — ska granskas av personuppgiftsansvarig.* |
| **Tredjelandsöverföring** | Cloudflare CDN (EU edge). Ingen persondata cachas. Mäträknare lagras i Cloudflare KV (EU edge), utan persondata. |
| **System** | frontend/member-portal (GREEN), frontend/wifi-intake-portal (GREEN), metrics-beacon `functions/m.ts` (YELLOW) |
